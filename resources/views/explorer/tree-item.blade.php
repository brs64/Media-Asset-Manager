    ...
    <div class="tree-item {{ $item['type'] }} mb-1">
        ...
        @if ($item['type'] === 'folder')
            <div class="dossier-label cursor-pointer text-gray-200 hover:text-white select-none flex items-center gap-2 group relative"
                ...
                @if (auth()->check() && auth()->user()->hasRole('admin') && $item['disk'] !== 'external_local')
                    ...
                    <div class="action-dropdown hidden fixed w-40 bg-gray-800 border border-gray-600 rounded shadow-2xl z-[1000] overflow-hidden hover:bg-blue-600">
